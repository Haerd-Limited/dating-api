...
func TestPreferenceMatcherMatchesAnyPreference(t *testing.T) {
	...
	datingID := int16(2)
	assert.True(t, matcher.matchesAny(50, 100, &datingID, nil, nil), "dating intention preference should match")

	religionID := int16(3)
	assert.True(t, matcher.matchesAny(50, 100, nil, &religionID, nil), "religion preference should match")
...
func TestPreferenceMatcherMatchesAllPreferences(t *testing.T) {
	...
	assert.True(t, matcher.matchesAll(30, 10, &datingID, &religionID, []int16{4}), "should match when all preferences satisfied")
	assert.False(t, matcher.matchesAll(30, 100, &datingID, &religionID, []int16{4}), "should fail distance preference")
	assert.False(t, matcher.matchesAll(36, 10, &datingID, &religionID, []int16{4}), "should fail age preference")
	assert.False(t, matcher.matchesAll(30, 10, nil, &religionID, []int16{4}), "should fail dating intention preference")
	assert.False(t, matcher.matchesAll(30, 10, &datingID, nil, []int16{4}), "should fail religion preference")
	assert.False(t, matcher.matchesAll(30, 10, &datingID, &religionID, []int16{5}), "should fail ethnicity preference")
}
