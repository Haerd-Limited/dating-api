...
func testConversationOneToOneRevealRequestUsingRevealRequest(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var foreign RevealRequest
	var local Conversation

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &foreign, revealRequestDBTypes, true, revealRequestColumnsWithDefault...); err != nil {
	...
	foreign.ConversationID = local.ID
	if err := foreign.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.RevealRequest().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}

	if check.ConversationID != foreign.ConversationID {
		t.Errorf("want: %v, got %v", foreign.ConversationID, check.ConversationID)
	}

	ranAfterSelectHook := false
	AddRevealRequestHook(boil.AfterSelectHook, func(ctx context.Context, e boil.ContextExecutor, o *RevealRequest) error {
		ranAfterSelectHook = true
		return nil
	})

	slice := ConversationSlice{&local}
	if err = local.L.LoadRevealRequest(ctx, tx, false, (*[]*Conversation)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.RevealRequest == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.RevealRequest = nil
	if err = local.L.LoadRevealRequest(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.RevealRequest == nil {
		t.Error("struct should have been eager loaded")
...
func testConversationOneToOneSetOpRevealRequestUsingRevealRequest(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, revealRequestDBTypes, false, strmangle.SetComplement(revealRequestPrimaryKeyColumns, revealRequestColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, revealRequestDBTypes, false, strmangle.SetComplement(revealRequestPrimaryKeyColumns, revealRequestColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*RevealRequest{&b, &c} {
		err = a.SetRevealRequest(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.RevealRequest != x {
			t.Error("relationship struct not set to correct value")
		}
		if x.R.Conversation != &a {
			t.Error("failed to append to foreign relationship struct")
		}

		if a.ID != x.ConversationID {
			t.Error("foreign key was wrong value", a.ID)
		}

		if exists, err := RevealRequestExists(ctx, tx, x.ConversationID); err != nil {
			t.Fatal(err)
		} else if !exists {
		...
		if a.ID != x.ConversationID {
			t.Error("foreign key was wrong value", a.ID, x.ConversationID)
		}
...
func testConversationToManyConversationParticipants(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadConversationParticipants(ctx, tx, false, (*[]*Conversation)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testConversationToManyRevealDecisions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := ConversationSlice{&a}
	if err = a.L.LoadRevealDecisions(ctx, tx, false, (*[]*Conversation)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testConversationToManyAddOpConversationParticipants(t *testing.T) {
	var err error
...
func testConversationToManyAddOpRevealDecisions(t *testing.T) {
	var err error
...
var (
	conversationDBTypes = map[string]string{`ID`: `uuid`, `UserA`: `uuid`, `UserB`: `uuid`, `CreatedAt`: `timestamp with time zone`, `LastActivityAt`: `timestamp with time zone`, `ALastReadID`: `bigint`, `BLastReadID`: `bigint`, `AMuted`: `boolean`, `BMuted`: `boolean`, `LastMessageID`: `bigint`, `VisibilityState`: `text`, `RevealAt`: `timestamp with time zone`}
	_                   = bytes.MinRead
)
