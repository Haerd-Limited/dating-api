...
func testQuestionsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(questionPrimaryKeyColumns, questionColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testQuestionToManyQuestionAnswers(t *testing.T) {
	...
	check, err := a.QuestionAnswers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadQuestionAnswers(ctx, tx, false, (*[]*Question)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadQuestionAnswers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.QuestionAnswers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testQuestionToManyUserAnswers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.UserAnswers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := QuestionSlice{&a}
	if err = a.L.LoadUserAnswers(ctx, tx, false, (*[]*Question)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserAnswers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UserAnswers = nil
	if err = a.L.LoadUserAnswers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserAnswers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testQuestionToManyAddOpQuestionAnswers(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddQuestionAnswers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		count, err := a.QuestionAnswers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testQuestionToManyAddOpUserAnswers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddUserAnswers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.UserAnswers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UserAnswers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UserAnswers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testQuestionToOneQuestionCategoryUsingCategory(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var local Question
	var foreign QuestionCategory

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, questionDBTypes, false, questionColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Question struct: %s", err)
	}
	if err := randomize.Struct(seed, &foreign, questionCategoryDBTypes, false, questionCategoryColumnsWithDefault...); err != nil {
	...
	AddQuestionCategoryHook(boil.AfterSelectHook, func(ctx context.Context, e boil.ContextExecutor, o *QuestionCategory) error {
		ranAfterSelectHook = true
		return nil
	})

	slice := QuestionSlice{&local}
	if err = local.L.LoadCategory(ctx, tx, false, (*[]*Question)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testQuestionToOneSetOpQuestionCategoryUsingCategory(t *testing.T) {
	var err error
	...
	var a Question
	var b, c QuestionCategory

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, questionDBTypes, false, strmangle.SetComplement(questionPrimaryKeyColumns, questionColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &b, questionCategoryDBTypes, false, strmangle.SetComplement(questionCategoryPrimaryKeyColumns, questionCategoryColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, questionCategoryDBTypes, false, strmangle.SetComplement(questionCategoryPrimaryKeyColumns, questionCategoryColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*QuestionCategory{&b, &c} {
...
var (
	questionDBTypes = map[string]string{`ID`: `bigint`, `CategoryID`: `bigint`, `Text`: `text`, `Type`: `text`, `IsActive`: `boolean`, `CreatedAt`: `timestamp with time zone`, `SortOrder`: `integer`}
	_               = bytes.MinRead
)
