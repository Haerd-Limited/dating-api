...
func testEventsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(eventPrimaryKeyColumns, eventColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testEventToOneUserUsingUser(t *testing.T) {
	...
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Event)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
...
func testEventToOneRemoveOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	eventDBTypes = map[string]string{`ID`: `uuid`, `OccurredAt`: `timestamp with time zone`, `UserID`: `uuid`, `SessionID`: `text`, `Name`: `text`, `Props`: `jsonb`, `Version`: `integer`}
	_            = bytes.MinRead
)
