...
func (interestL) LoadUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeInterest interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"users\".\"id\", \"users\".\"email\", \"users\".\"phone\", \"users\".\"password_hash\", \"users\".\"created_at\", \"users\".\"updated_at\", \"a\".\"interest_id\""),
		qm.From("\"users\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Email, &one.Phone, &one.PasswordHash, &one.CreatedAt, &one.UpdatedAt, &localJoinCol)
		if err != nil {
