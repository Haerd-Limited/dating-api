...
var (
	matchDBTypes = map[string]string{`ID`: `uuid`, `UserA`: `uuid`, `UserB`: `uuid`, `CreatedAt`: `timestamp with time zone`, `RevealedAt`: `timestamp with time zone`, `Status`: `enum.match_status('active','unmatched','blocked')`, `DateMode`: `boolean`, `UnmatchReason`: `text`}
	_            = bytes.MinRead
)
