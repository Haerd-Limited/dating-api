...
func testMatchToManyMessagesVoices(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.MessagesVoices().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := MatchSlice{&a}
	if err = a.L.LoadMessagesVoices(ctx, tx, false, (*[]*Match)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.MessagesVoices); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.MessagesVoices = nil
	if err = a.L.LoadMessagesVoices(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.MessagesVoices); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testMatchToManyAddOpMessagesVoices(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddMessagesVoices(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.MessagesVoices[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.MessagesVoices[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.MessagesVoices().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
