...
func testUserBlocksExists(t *testing.T) {
	...
	e, err := UserBlockExists(ctx, tx, o.BlockerUserID, o.BlockedUserID)
	if err != nil {
...
func testUserBlocksFind(t *testing.T) {
	...
	userBlockFound, err := FindUserBlock(ctx, tx, o.BlockerUserID, o.BlockedUserID)
	if err != nil {
		t.Error(err)
	}
...
func testUserBlocksInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(userBlockPrimaryKeyColumns, userBlockColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testUserBlockToOneUserUsingBlockedUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	local.BlockedUserID = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.BlockedUser().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadBlockedUser(ctx, tx, false, (*[]*UserBlock)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.BlockedUser == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.BlockedUser = nil
	if err = local.L.LoadBlockedUser(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.BlockedUser == nil {
		t.Error("struct should have been eager loaded")
...
func testUserBlockToOneUserUsingBlockerUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	local.BlockerUserID = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.BlockerUser().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserBlockSlice{&local}
	if err = local.L.LoadBlockerUser(ctx, tx, false, (*[]*UserBlock)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.BlockerUser == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.BlockerUser = nil
	if err = local.L.LoadBlockerUser(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.BlockerUser == nil {
		t.Error("struct should have been eager loaded")
...
func testUserBlockToOneSetOpUserUsingBlockedUser(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		err = a.SetBlockedUser(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.BlockedUser != x {
			t.Error("relationship struct not set to correct value")
		}

		if x.R.BlockedUserUserBlocks[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.BlockedUserID != x.ID {
			t.Error("foreign key was wrong value", a.BlockedUserID)
		}

		if exists, err := UserBlockExists(ctx, tx, a.BlockerUserID, a.BlockedUserID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
func testUserBlockToOneSetOpUserUsingBlockerUser(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		err = a.SetBlockerUser(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.BlockerUser != x {
			t.Error("relationship struct not set to correct value")
		}

		if x.R.BlockerUserUserBlocks[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.BlockerUserID != x.ID {
			t.Error("foreign key was wrong value", a.BlockerUserID)
		}
...
var (
	userBlockDBTypes = map[string]string{`BlockerUserID`: `uuid`, `BlockedUserID`: `uuid`, `Reason`: `text`, `CreatedAt`: `timestamp with time zone`}
	_                = bytes.MinRead
)
