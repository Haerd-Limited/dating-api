...
func testUserReportToManyReportReportActions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.ReportReportActions().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if v.ReportID == b.ReportID {
			bFound = true
		}
		if v.ReportID == c.ReportID {
			cFound = true
		}
	}
	...
	if err = a.L.LoadReportReportActions(ctx, tx, false, (*[]*UserReport)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ReportReportActions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ReportReportActions = nil
	if err = a.L.LoadReportReportActions(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ReportReportActions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserReportToManyAddOpReportReportActions(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddReportReportActions(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ReportReportActions[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ReportReportActions[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ReportReportActions().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserReportToOneUserUsingReportedUser(t *testing.T) {
	...
	local.ReportedUserID = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.ReportedUser().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadReportedUser(ctx, tx, false, (*[]*UserReport)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ReportedUser == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.ReportedUser = nil
	if err = local.L.LoadReportedUser(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ReportedUser == nil {
		t.Error("struct should have been eager loaded")
...
func testUserReportToOneUserUsingReporterUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	local.ReporterUserID = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.ReporterUser().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserReportSlice{&local}
	if err = local.L.LoadReporterUser(ctx, tx, false, (*[]*UserReport)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ReporterUser == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.ReporterUser = nil
	if err = local.L.LoadReporterUser(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ReporterUser == nil {
		t.Error("struct should have been eager loaded")
...
func testUserReportToOneSetOpUserUsingReportedUser(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		err = a.SetReportedUser(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.ReportedUser != x {
			t.Error("relationship struct not set to correct value")
		}

		if x.R.ReportedUserUserReports[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.ReportedUserID != x.ID {
			t.Error("foreign key was wrong value", a.ReportedUserID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.ReportedUserID))
		reflect.Indirect(reflect.ValueOf(&a.ReportedUserID)).Set(zero)
		...
		if a.ReportedUserID != x.ID {
			t.Error("foreign key was wrong value", a.ReportedUserID, x.ID)
		}
	}
}
func testUserReportToOneSetOpUserUsingReporterUser(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		err = a.SetReporterUser(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.ReporterUser != x {
			t.Error("relationship struct not set to correct value")
		}

		if x.R.ReporterUserUserReports[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.ReporterUserID != x.ID {
			t.Error("foreign key was wrong value", a.ReporterUserID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.ReporterUserID))
		reflect.Indirect(reflect.ValueOf(&a.ReporterUserID)).Set(zero)
		...
		if a.ReporterUserID != x.ID {
			t.Error("foreign key was wrong value", a.ReporterUserID, x.ID)
		}
	}
}
...
var (
	userReportDBTypes = map[string]string{`ID`: `uuid`, `ReporterUserID`: `uuid`, `ReportedUserID`: `uuid`, `SubjectType`: `text`, `SubjectID`: `uuid`, `Category`: `text`, `Notes`: `text`, `Status`: `enum.report_status('pending','in_review','resolved','escalated','dismissed')`, `Severity`: `text`, `Metadata`: `jsonb`, `AutoAction`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `ResolvedAt`: `timestamp with time zone`}
	_                 = bytes.MinRead
)
