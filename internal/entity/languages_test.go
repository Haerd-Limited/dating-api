...
func testLanguagesInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(languagePrimaryKeyColumns, languageColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testLanguageToManyUsers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into \"user_languages\" (\"language_id\", \"user_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"user_languages\" (\"language_id\", \"user_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadUsers(ctx, tx, false, (*[]*Language)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testLanguageToManyAddOpUsers(t *testing.T) {
	var err error
...
func testLanguageToManySetOpUsers(t *testing.T) {
	var err error
...
func testLanguageToManyRemoveOpUsers(t *testing.T) {
	var err error
...
var (
	languageDBTypes = map[string]string{`ID`: `smallint`, `Code`: `text`, `Label`: `text`}
	_               = bytes.MinRead
)
